...
namespace IncidentAlert.Repositories.Implementation
{
    public class LocationRepository(DataContext dataContext) : ILocationRepository
    {
        ...
        public async Task<IEnumerable<Location>> Find(Expression<Func<Location, bool>> predicate) => await _dataContext.Locations.Where(predicate).ToListAsync();


        public async Task<IEnumerable<Location>> GetAll() => await _dataContext.Locations.ToListAsync();

        public async Task<Location?> GetById(int id) => await _dataContext.Locations.FirstOrDefaultAsync(e => e.Id == id);
